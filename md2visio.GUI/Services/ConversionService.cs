...
{
    /// <summary>
    /// Mermaid to Visio conversion service - uses the new API layer
    /// </summary>
    public class ConversionService : IDisposable
    {
            ...
            bool silentOverwrite)
        {
            try
            {
                ...
                var request = new md2visio.Api.ConversionRequest(
                    ...
                    debug: true  // Keep debug mode for detailed logging
                );
                ...
                lock (_lock)
                {
                    if (showVisio)
                    {
                        // Show mode: reuse converter to keep Visio window open
                        _converter ??= new md2visio.Api.Md2VisioConverter();
                    }
                    else
                    {
                        // Non-show mode: create a new converter each time
                        _converter?.Dispose();
                        _converter = new md2visio.Api.Md2VisioConverter();
                    }
                }
                ...
                if (apiResult.Success)
                {
                    ...
                    ReportLog($"Successfully generated {apiResult.OutputFiles.Length} file(s):");
            ...
            catch (System.Runtime.InteropServices.COMException ex)
            {
                ...
                return ConversionResult.Error($"COM component error. Possible causes:\n" +
                    "1. Microsoft Visio is not correctly installed or registered\n" +
                    "2. Visio process is locked or insufficient permissions\n" +
                    "3. System COM components are corrupted\n" +
        ...
        /// <summary>
        /// Check whether Visio is available
        /// </summary>
        public ConversionResult CheckVisioAvailability()
        {
            ...
            catch (System.Runtime.InteropServices.COMException ex)
            {
                ...
                return ConversionResult.Error($"Visio environment check failed:\n" +
                    "1. Please ensure Microsoft Visio is correctly installed\n" +
                    "2. Check that Visio is properly registered\n" +
    ...
    /// <summary>
    /// Conversion result (kept for backward compatibility)
    /// </summary>
