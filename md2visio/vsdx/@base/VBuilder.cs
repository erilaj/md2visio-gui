...
namespace md2visio.vsdx.@base
{
    internal abstract class VBuilder
    {
        ...
        public void SaveAndClose(string outputFile)
        {
            ...
            // If showing the Visio window, give the user time to view the result
        ...
        bool CanWriteOutputFile(string outputFile, out string? reason)
        {
            ...
            {
                reason = $"Output file is read-only or locked, cannot write: {outputFile}";
                return false;
            }
            catch (IOException)
            {
                reason = $"Output file is being used by another process, please close it and retry: {outputFile}";
                return false;
            }
        }
