...
namespace md2visio.vsdx
{
    ...
    {
        // Layout parameters — these values are in 'layout units' (mm * LayoutScale)
        // When writing to Visio they are uniformly converted to mm with a unit suffix
        ...
        private double bottomY;                  // Bottom participant Y position (computed dynamically)
        private double diagramStartY;            // Diagram content start Y position (computed dynamically)

        // ====== Coordinate unit conversion helpers ======
        // Visio internal units are inches; a "mm" suffix is required for correct millimeter interpretation

        /// <summary>
        /// Format a number as a Visio-formula-compatible string (using invariant culture to avoid comma issues)
        /// </summary>
        private static string Num(double value)
            => value.ToString("0.###############", CultureInfo.InvariantCulture);

        /// <summary>
        /// Convert a layout unit value to a Visio formula string with mm suffix
        ...
        /// <summary>
        /// Convert layout units to Visio internal units (inches)
        ...
        /// message.Y and activation.CenterY are relative values; add diagramStartY offset
        /// </summary>
        private double AbsY(double relativeY) => diagramStartY + relativeY;

        /// <summary>
        /// Move a shape to the specified layout coordinate (with mm unit)
        /// </summary>
        private void PlaceAt(Shape shape, double xLayoutUnits, double yLayoutUnits)
            => MoveTo(shape, Mm(xLayoutUnits), Mm(yLayoutUnits));

        /// <summary>
        /// Debug log: output the actual position of the shape
        /// </summary>
        ...
        public override void Draw()
        {
            ...
            if (_context.Debug)
            {
                _context.Log($"[DEBUG] VDrawerSeq: Starting sequence diagram drawing, participant count: {figure.Participants.Count}, message count: {figure.Messages.Count}, activation count: {figure.Activations.Count}");
            }
            ...
            try
            {
                ...
                if (_context.Debug)
                {
                    _context.Log($"[DEBUG] VDrawerSeq: Starting layout calculation");
                }
                CalculateLayout();
                ...
                if (_context.Debug)
                {
                    _context.Log($"[DEBUG] VDrawerSeq: Starting participant drawing");
                }
                DrawParticipants();
                PauseForViewing(500);
                ...
                if (_context.Debug)
                {
                    _context.Log($"[DEBUG] VDrawerSeq: Starting lifeline drawing");
                }
                DrawLifelines();
                PauseForViewing(300);
                ...
                if (_context.Debug)
                {
                    _context.Log($"[DEBUG] VDrawerSeq: Starting activation box drawing");
                }
                DrawActivations();
                PauseForViewing(300);

                // 5. Draw combined fragments
                if (_context.Debug)
                {
                    _context.Log($"[DEBUG] VDrawerSeq: Starting combined fragment drawing");
                }
                DrawFragments();
                PauseForViewing(300);
                ...
                if (_context.Debug)
                {
                    _context.Log($"[DEBUG] VDrawerSeq: Starting note drawing");
                }
                DrawNotes();
                PauseForViewing(300);
                ...
                if (_context.Debug)
                {
                    _context.Log($"[DEBUG] VDrawerSeq: Starting message drawing");
                }
                DrawMessages();
                PauseForViewing(500);
                ...
                if (_context.Debug)
                {
                    _context.Log($"[DEBUG] VDrawerSeq: Sequence diagram drawing complete");
                }
            }
            catch (Exception ex)
            {
                if (_context.Debug)
                {
                    _context.Log($"[DEBUG] VDrawerSeq: Exception occurred during drawing: {ex.Message}");
                    ...
                    _context.Log($"[DEBUG] VDrawerSeq: Exception stack trace: {ex.StackTrace}");
                }
                throw;
            }
        }
        ...
        {
            // Calculate participant horizontal positions (scaled up 15x)
            double startX = 1500;
        ...
        {
            // 1. Calculate message start Y: immediately below the top participant box
            double messageStartOffset = messageSpacing / 2;
            diagramStartY = topY - participantHeight / 2 - messageStartOffset;

            // 2. Find the lowest content point (minimum relative Y value)
            // Visio coordinate system Y axis grows upward, so lowest content = minimum Y value
            ...
            // 3. Edge case: set minimum height when there is no content
            ...
            // 4. Dynamically calculate the bottom participant position
        ...
        private void DrawParticipants()
        {
            ...
            {
                // Top participant - create at (0,0) first, then move to correct position
        ...
        private void DrawLifelines()
        {
            foreach (var participant in figure.Participants)
            {
                // Get the actual participant box height (AdjustSize may have adjusted it)
        ...
        {
            // Use DrawLine to draw a simple 1D line, avoiding dynamic connector snapping behavior
        ...
        private void DrawActivations()
        {
            foreach (var activation in figure.Activations)
            {
                ...
                // Create at (0,0) first, then move to correct position
        ...
        private Shape DrawRegularMessage(SeqMessage message)
        {
            ...
            // Always draw the line left-to-right to prevent text flipping
            Shape messageShape = visioPage.DrawLine(
                Inches(leftX), Inches(y),
                Inches(rightX), Inches(y));

            // Set arrow position according to actual direction
        ...
        private void SetupMessageArrowWithDirection(Shape messageShape, string arrowType, bool isLeftToRight)
        {
            ...
            // The line is always drawn left-to-right, so:
            // - Left-to-right message: arrow at End (right side)
            // - Right-to-left message: arrow at Begin (left side)
        ...
        private Shape DrawSelfCallMessage(SeqMessage message)
        {
            ...
            // Use DrawLine API to draw three line segments forming a U-shaped self-call
            // 1. Horizontal line going right
            Shape line1 = visioPage.DrawLine(x1, y1, x2, y1);
            // 2. Vertical line going down
            Shape line2 = visioPage.DrawLine(x2, y1, x2, y2);
            // 3. Horizontal line going left (with arrow)
            ...
            return line3; // Return the line that has the arrow
        }
