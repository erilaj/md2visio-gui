...
namespace md2visio.Api
{
    ...
    /// Wraps existing conversion logic and provides a clean API.
    /// </summary>
    public sealed class Md2VisioConverter : IMd2VisioConverter
    {
            ...
            ILogSink logger)
        {
            ...
            // Step 6: Clean up if not showing Visio
            ...
            progress?.Report(new ConversionProgress(100, "Conversion complete!", ConversionPhase.Completed));

            var outputFiles = CollectOutputFiles(request);

            if (outputFiles.Length > 0)
            {
                logger.Info($"Generated {outputFiles.Length} file(s):");
            ...
            else
            {
                ...
                if (factory.FiguresBuilt == 0)
                {
                    ...
                    if (factory.UnsupportedTypes.Count > 0)
                    {
                        ...
                        return ConversionResult.Failed(
                            $"File contains unsupported diagram type(s): {unsupported}\n" +
                            $"Currently supported: {supportedTypes}");
                    }
                    else
                    {
                        return ConversionResult.Failed(
                            "No valid Mermaid diagrams were found in the file.\n" +
                            "Please ensure diagrams are enclosed in ```mermaid ... ``` blocks.");
                    }
                }

                logger.Warning("Conversion completed but no output files were found.");
                return ConversionResult.Failed("Conversion completed but no output files were generated. Please check the output path and permissions.");
            }
        }
        ...
        private string[] CollectOutputFiles(ConversionRequest request)
        {
            ...
            {
                // File mode: check the specified file
