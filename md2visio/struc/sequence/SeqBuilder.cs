...
namespace md2visio.struc.sequence
{
    internal class SeqBuilder : FigureBuilder
    {
        ...
        public override void Build(string outputFile)
        {
            if (_context.Debug)
            {
                _context.Log($"[DEBUG] SeqBuilder.Build: Starting build, output file: {outputFile}");
            }
            ...
            while (iter.HasNext())
            {
                ...
                else if (cur is SttMermaidClose)
                {
                    if (_context.Debug)
                    {
                        _context.Log($"[DEBUG] SeqBuilder: Parse complete, participant count: {sequence.Participants.Count}, message count: {sequence.Messages.Count}");
                        _context.Log($"[DEBUG] SeqBuilder: Activation count: {sequence.Activations.Count}");
                        ...
                        _context.Log($"[DEBUG] SeqBuilder: Calling sequence.ToVisio(\"{outputFile}\")");
                    }
                    ...
                    try
                    {
                        ...
                        if (_context.Debug)
                        {
                            _context.Log($"[DEBUG] SeqBuilder: sequence.ToVisio() call complete");
                        }
                    }
                    catch (Exception ex)
                    {
                        if (_context.Debug)
                        {
                            ...
                            _context.Log($"[DEBUG] SeqBuilder: Exception stack trace: {ex.StackTrace}");
                        }
                        throw;
                    }
                    break;
        ...
        private void BuildParticipant()
        {
            if (_context.Debug)
            {
                _context.Log($"[DEBUG] SeqBuilder.BuildParticipant: Starting participant processing");
            }
        ...
        private void ParseMessageLine(string messageText)
        {
            ...
            if (TryParseMessage(messageText, out string from, out string to, out string arrowType, out string activationOp, out string label))
            {
                if (_context.Debug)
                {
                    _context.Log($"[DEBUG] SeqBuilder: Parse successful - From: {from}, To: {to}, Arrow: {arrowType}, Label: {label}");
                }
